...
void init_signals_handling()
{
    ...
    sigset_t set;
    sigemptyset(&set);
    sigaddset(&set, SIGALRM);   //сист таймер
    sigaddset(&set, SIGUSR1);
    sigaddset(&set, SIGUSR2);

    action.sa_flags = SA_SIGINFO;   //флаг передачи доп инфы
    action.sa_mask = set;   //маска сигналов, блокирующихся во время обработки
    action.sa_sigaction = signal_handler;   //установка обработчика

    sigaction(SIGALRM, &action, NULL);  //установка обработчика на каждый сигнал
...
void case_plus()
{
    ...
    if (pid == 0) execl(child_name, child_name, NULL);  //дочерний процесс  GR
    else
    {
        child_processes_size++;
...
void case_g(int index)
{
    ...
    for(int i = 0; i < (int) child_processes_size; i++)
    {
        ...
        printf("%s with pid = %d is running no./w\n", child_processes[i].name, child_processes[i].pid);
    }
}
